...
export default function App() {
  ...
  const submitImage = async (event: React.FormEvent) => {
    ...
    try {
      ...
      console.log(`Success! Uploaded: ${data.process}, ${data.imageName}, ${data.result}`);
      setRes(data.result)
    } catch (error) {
  ...
  return (
    <div className=" h-auto w-auto p-8 bg-white rounded-md shadow-xl flex flex-col items-center">
      <h1 className ="text-black flex justify-start text-2xl mb-2">Classfiication Models</h1>
      <form onSubmit={submitImage} className ="flex flex-col self-start space-y-4">
      <div className="mb-4">
          <label htmlFor="process" className="mr-2 text-black">Select Model</label>
          <select
            id="process"
            value={process}
        ...
        <input
          ...
          required
          className= {`mb-2 text-black border ${uploaded ? 'border-green-500' : 'border-grey-300'} p-2 rounded $`}
        />
        
        <button type="submit" className="bg-green-600 text-white py-2 px-4 rounded hover:text-green-500 hover:bg-white transition-all durration-200 rounded border border-transparent hover:border-green-500 ">
          Process Image
        </button>
      </form>
      ...
      {
        res.length>0 && (
          <div className = "flex justify-center items-center p-4 text-green-500 text-xl hover:scale-110">
            {`Results : ${res}`}
            </div>
        )
      }
